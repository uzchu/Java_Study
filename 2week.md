...
> - 데이터 손실 존재할 수 있음
> - 캐스팅 연산자를 명시적으로 작성해야 함
>
> **(Promotion, 프로모션, 묵시적 형변환)**
>
> - 작은 데이터 타입에서 큰 데이터 타입으로 형변환
> - 데이터 손실 없음
> - 하위클래스에서 상위 클래스 메소드를 오버라이드하면 하위 메소드가 호출됨
>   - 예외 : 상위 클래스의 static 메소드는 하위 클래스에서 오버라이드 불가능

- ref : https://castleone.tistory.com/5
...
>   - new를 통한 키워드가 아닌 {}를 통해 배열을 초기화 할 수도 있음
>   - 배열 인덱스의 시작은 0부터
>   - new int[10]을 했다면 인덱스는 0~9까지 존재하게 됨
>3. 2차원 배열 선언
>   - 2차원 배열은 대괄호([])가 2개 생김
>   - 첫번째 대괄호는 행, 두번째 대괄호는 열의 숫자
...
>**타입추론**
>
>- 코드 작성 당시 정해지진 않았지만, 컴파일러가 그 타입을 유추하는 것
>- 변수 타입을 선언하는것이 아니라 설정되는 값에서 유추함
>
>**var**
>
>- 자바10에 생긴 키워드이며 타입추론을 지원함
